...
- The user still has `C:\Users\<user>\Desktop\Jhn Apps\jhn-twitch-viewer.previous\` sitting on their Desktop with ~26GB of orphaned `.ts` recordings in its `temp/` folder (leftovers from the old script force-killing the app mid-recording during past updates). The new script will never touch this folder again, but it hasn't been cleaned up or reviewed for anything worth keeping.
