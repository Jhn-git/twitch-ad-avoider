...
class ErrorRecoveryManager:
    ...
    def record_error(self, error: Exception, operation: str, channel: Optional[str] = None) -> ErrorType:
        """
    ...
    def get_error_statistics(self) -> Dict:
        ...
        if total_errors == 0:
            return {"total": 0, "by_type": {}, "consecutive": 0, "condition": self._network_condition.value}
    ...
    def _classify_error(self, error: Exception) -> ErrorType:
        ...
        # Connection errors
        if any(keyword in error_message for keyword in ["connection refused", "connection reset", "connection failed"]):
            return ErrorType.CONNECTION_REFUSED
            
        # DNS errors
        if any(keyword in error_message for keyword in ["name resolution", "dns", "host not found"]):
